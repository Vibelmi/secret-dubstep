...
if (isset($_SESSION['email'])) {
    ?>
    <button id="btnlogout" class="btnlogin"><?php echo $cont->logout; ?> | <?php echo $_SESSION['name']; ?></button>
    <script>include("modules/login/js/logout.js");</script>
    <?php
} else {
    ?>
    <script>
        include("modules/login/js/jquery.lightbox_me.js");
        include("modules/login/js/login.js");
    </script>
    <button id="btnlogin" class="btnlogin"><?php echo $cont->login ?></button>

    <div id="login">
        <img id="ximg" height="20" width="20" src="modules/login/images/x.png" alt="x">
        <h2 class="loginh2"><?php echo $cont->login ?></h2>
        <div>
            <input id="email_login" type="email" class="logininput" placeholder="<?php echo $cont->email ?>"><br><br>
